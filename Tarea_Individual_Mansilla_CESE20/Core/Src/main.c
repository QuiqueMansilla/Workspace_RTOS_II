...
int main(void)
{
  ...
  /* USER CODE BEGIN 2 */
  uartReceiveIT(datos_RX, 1);//tercer parametro es la cantidad de datos que recibira para que se produzca la interrupcion: 1
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
  ...
  {
	  uint8_t dato = datos_RX[0];
	  dato_Recib[indice++] = dato;
	  if (dato = '*') //identifica inicio de trama
	  {
		  indice = 0;
	  }
	  if (indice >= SIZE_RX)
   ...
   */
  uartSendStringSize(&dato_Recib, SIZE_RX);
  uartReceiveIT(datos_RX, 1); // para que vuelva a llamar a la interrupcion.
}
/* USER CODE END 4 */
